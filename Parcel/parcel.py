...
def interpolate(lol, n=10, t="linear"):
    if len(lol) == n:
        return lol
    elif len(lol) < n:
        final_x = []
...
H = model.fit(np.asarray(X[:-50]), np.asarray(y[:-50]), validation_data=(X[50:], y[50:]), batch_size=16, epochs=10, verbose=1, callbacks=callbacks)
